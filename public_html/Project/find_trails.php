...
if (isset($_GET["find"])) {
    ...
    switch ($type) {
        case "location":
            ...
            break;
        case "other":
            if (!isset($_GET["name"]) || !isset($_GET["difficulty"]) || !isset($_GET["country"])) {
                flash("Name, country or difficulty is not set.", "danger");
                $hasError = true;
            }
            break;
        default:
    ...
    if ($type === "other" && !$hasError) {
        ...
        if (empty($name) && empty($country) && empty($diff)) {
            $hasError = true;
            flash("You must specify at least one field (name, country, or difficulty).", "warning");
        }
        ...
        if (!empty($name)) {
            if (strlen($name) <= 0 || strlen($name) > 50) {
                $hasError = true;
                flash("The trail name must be 50 characters or less.", "danger");
            } else {
                $search_name = true;
            }
        }
                ...
                <div class="container-sm p-5 rounded-2" style="background-color: #ffffff;">
                    ...
                    <form method="GET" onsubmit="return validate(this);">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" id="lat" class="form-control" name="lat" placeholder="ex. -46.325" aria-describedby="basic-addon1">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">Longitude:</span>
                            <input type="text" id="long" class="form-control" name="long" placeholder="ex. 100.453" aria-describedby="basic-addon1">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">Radius:</span>
                            <input type="text" id="radius" class="form-control" name="radius" placeholder="In miles, ex. 5" aria-describedby="basic-addon1">
                        </div>
                        <div class="input-group mb-3">
                        ...
                        <tbody>
                            <?php foreach ($result as $trail) : ?>
                                <tr>
                                    ...
                                    <?php if ($_GET["find"] == "location"): ?>
                                    <td><?php se($trail, "distance", "", true); ?></td>
                                    <?php endif ?>
    ...
    function validate(form) {
        ...
        if (form.find.value == "location") {
            ...
            if (country == "" && name == "" && diff == "") {
                flash("You must specify a field between name, country or difficulty.", "warning");
                return false;
            }
            ...
            if (name !== "" && name.length > 50) {
                flash("Please enter a trail name that is less than 50 characters.", "warning");
                return false;
            }
