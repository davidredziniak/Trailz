...
function get_favorites_by_user_id($id){
    $db = getDB();
    $stmt = $db->prepare("SELECT t.id, t.name, t.country, t.difficulty, unix_timestamp(u.created) AS created FROM `User_Favorites` AS u JOIN Trails AS t ON u.trail_id = t.id WHERE u.user_id=:id;");
    try {
        $stmt->execute([":id" => intval($id)]);
        $r = $stmt->fetchAll();
        if($r){
